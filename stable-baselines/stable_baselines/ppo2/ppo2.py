...
class PPO2(ActorCriticRLModel):
    ...
    def learn(self, total_timesteps, run, callback=None, log_interval=1, tb_log_name="PPO2",
        ...
        with SetVerbosity(self.verbose), TensorboardWriter(self.graph, self.tensorboard_log, tb_log_name, new_tb_log) \
            ...
            for update in range(1, n_updates + 1):
                ...
                if run is not None and (update % log_interval == 0 or update == 1):
                    explained_var = explained_variance(values, returns)
                    run["train/serial_timesteps"].log(update * self.n_steps)
                    run["train/n_updates"].log(update)
                    ...
                    run["train/time_elapsed"].log(t_start - t_first_start)
                    for (loss_val, loss_name) in zip(loss_vals, self.loss_names):
                        run["train/{}".format(loss_name)].log(loss_val)
