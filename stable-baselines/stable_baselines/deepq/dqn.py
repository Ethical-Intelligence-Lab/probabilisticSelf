...
class DQN(OffPolicyRLModel):
    ...
    def learn(self, total_timesteps, run, callback=None, log_interval=1, tb_log_name="DQN",
        ...
        with SetVerbosity(self.verbose), TensorboardWriter(self.graph, self.tensorboard_log, tb_log_name, new_tb_log) \
            ...
            for _ in range(total_timesteps):
                ...
                if done and log_interval is not None and len(episode_rewards) % log_interval == 0 and run is not None:
                    run["train/steps"].log(self.num_timesteps)
                    run["train/episodes"].log(num_episodes)
                    run["train/mean_100_episode_reward"].log(mean_100ep_reward)
                    run["train/time_spent_exploring"].log(int(100 * self.exploration.value(self.num_timesteps)))
                    if len(episode_successes) > 0:
                        run["train/success_rate"].log(np.mean(episode_successes[-100:]))
